...
class Annotator(tk.Tk):
    def __init__(self):
        ...
        # Panel boczny
        side = ttk.Frame(main); side.grid(row=0, column=1, sticky="nsew", padx=6, pady=6)
        ttk.Label(side, text="Klasa (1/2/3):").pack(anchor="w")
        self.cls_var = tk.StringVar(value=CLASSES[0])
        self.cls_combo = ttk.Combobox(side, textvariable=self.cls_var, values=CLASSES, state="readonly")
        ...
        ttk.Button(btns, text="Usuń (Del)", command=self.delete_selected).pack(side="left", padx=(0,6))
        ttk.Button(btns, text="Zapisz (S)", command=self.save_labels).pack(side="left")

        # Status
        self.status = tk.StringVar()
        ttk.Label(self, textvariable=self.status).grid(row=1, column=0, sticky="we", padx=6, pady=(0,6))
    ...
    def img_to_canvas_coords(self, x, y):
        ...
        if not self.curr_image: return x, y
    ...
    def canvas_to_img_coords(self, X, Y):
        ...
        if not self.curr_image: return 0, 0
        W, H = self.curr_image.size
        cw = int(W * self.zoom)
        ch = int(H * self.zoom)
        x0 = (self.canvas.winfo_width() - cw) // 2
    ...
    def on_release(self, e):
        ...
        W, H = self.curr_image.size
        lx1, lx2 = sorted([x1, x2]); ly1, ly2 = sorted([y1, y2])
        w = (lx2 - lx1) / W; h = (ly2 - ly1) / H
        cx = (lx1 + lx2) / 2 / W; cy = (ly1 + ly2) / 2 / H
    ...
    def on_select_box(self, _):
        self.sel_box = self.listbox.curselection()[0] if self.listbox.curselection() else None
        self.redraw()

    def on_key(self, e):
        if e.char == "1": self.cls_var.set(CLASSES[0]); self.set_status()
        elif e.char == "2": self.cls_var.set(CLASSES[1]); self.set_status()
        elif e.char == "3": self.cls_var.set(CLASSES[2]); self.set_status()
        elif e.char.lower() == "s": self.save_labels()
        elif e.char.lower() == "a": self.prev_img()
        elif e.char.lower() == "d": self.next_img()
        elif e.char.lower() == "h": messagebox.showinfo("Help", HELP)
        elif e.keysym == "Delete": self.delete_selected()
        elif (e.state & 4) and e.keysym in ("plus", "KP_Add"):  # Ctrl + +
            self.zoom = min(4.0, self.zoom * 1.1); self.redraw(); self.set_status()
        elif (e.state & 4) and e.keysym in ("minus", "KP_Subtract"):
            self.zoom = max(0.25, self.zoom / 1.1); self.redraw(); self.set_status()
        ...
        if y.exists():
            with open(y, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if len(parts) == 5:
                        cls, cx, cy, w, h = parts
                        self.boxes.append([int(cls), float(cx), float(cy), float(w), float(h)])
        self.refresh_list()
        self.redraw()
        self.set_status()
    ...
    def redraw(self, temp_rect=None):
        ...
        for i, (cls, cx, cy, w, h) in enumerate(self.boxes):
            ...
            self.canvas.create_text(X1+4, Y1+10, text=CLASSES[cls], fill=color, anchor="w")

        # rysuj aktualnie przeciągany prostokąt (podgląd)
        if temp_rect:
            (sx, sy), (ex, ey) = temp_rect
            self.canvas.create_rectangle(sx, sy, ex, ey, outline="#ffffff", dash=(4,2), width=2)
    ...
    def refresh_list(self):
        ...
        for cls, cx, cy, w, h in self.boxes:
            self.listbox.insert(tk.END, f"{CLASSES[cls]}  cx={cx:.3f} cy={cy:.3f} w={w:.3f} h={h:.3f}")
